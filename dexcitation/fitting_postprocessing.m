...
%%
% Read actual rates
actualRates = readmatrix('rates_N4_59_DEXh_5T.dat'); % Adjust filename and format as needed

%%

% Assuming the temperatures of your 5 files are stored in a vector 'temperatures'
temperatures = [6000, 10000, 13000, 20000, 25000]; % Replace with actual temperatures

%% 

%%%%%%%%%%%%%%%%%%%%%%%%% Read parameters file %%%%%%%%%%%%%%%%%%%%%%%%%%%%
fid = fopen('fit_DExh_N4_59_5T.dat', 'r');
...
rmsErrors = sqrt(mean((fitted./actualRates(:, 6:end) - 1).^2, 2));
...
for i = 1:n
    ...
    E_prod = (energy_levels(actualRates(i,3),2)) + diss_energy_N2;
...
process_1 = sprintf('process %d: N2(%d) + N2(%d) = N2(%d) + 2N',topFiveIndices(1),actualRates(topFiveIndices(1),1), actualRates(topFiveIndices(1),2), actualRates(topFiveIndices(1),3));
process_2 = sprintf('process %d: N2(%d) + N2(%d) = N2(%d) + 2N',topFiveIndices(2),actualRates(topFiveIndices(2),1), actualRates(topFiveIndices(2),2), actualRates(topFiveIndices(2),3));
process_3 = sprintf('process %d: N2(%d) + N2(%d) = N2(%d) + 2N',topFiveIndices(3),actualRates(topFiveIndices(3),1), actualRates(topFiveIndices(3),2), actualRates(topFiveIndices(3),3));
process_4 = sprintf('process %d: N2(%d) + N2(%d) = N2(%d) + 2N',topFiveIndices(4),actualRates(topFiveIndices(4),1), actualRates(topFiveIndices(4),2), actualRates(topFiveIndices(4),3));
process_5 = sprintf('process %d: N2(%d) + N2(%d) = N2(%d) + 2N',topFiveIndices(5),actualRates(topFiveIndices(5),1), actualRates(topFiveIndices(5),2), actualRates(topFiveIndices(5),3));
